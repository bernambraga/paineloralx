...
import "./comercialGeral.css"; // Importe o CSS aqui
...
const ComercialGeral = () => {
  ...
  const examesAnoAtual = graficoGeral.exames_mensal?.filter(
    (item) => item.ano === new Date().getFullYear()
  );
  const examesAnoAnterior = graficoGeral.exames_mensal?.filter(
    (item) => item.ano === new Date().getFullYear() - 1
  );
    ...
    datasets: [
      {
        ...
        data: (() => {
          ...
          const examesFiltradosAnoAtual = filtrarDadosPorModalidade(
            examesAnoAtual,
      ...
      },
      {
        ...
        data: (() => {
          ...
          const examesFiltradosAnoAnterior = filtrarDadosPorModalidade(
            examesAnoAnterior,
            modalidadesSelecionadas
          );
          ...
          return meses.map((_, index) => {
            if (index === 0) {
              const totalExamesMesAnterior = examesFiltradosAnoAnterior
                .filter((item) => getMonthIndex(item.mes) === 11)
                .reduce((acc, item) => acc + (item.total_exames || 0), 0);
              return totalExamesMesAnterior === 0
                ? null
                : totalExamesMesAnterior;
            } else {
              const totalExamesMesAnterior = examesFiltradosAnoAnterior
                .filter((item) => getMonthIndex(item.mes) + 1 === index)
                .reduce((acc, item) => acc + (item.total_exames || 0), 0);
  ...
  const pacientesAnoAtual = graficoGeral.exames_mensal?.filter(
    (item) => item.ano === new Date().getFullYear()
  );
  const pacientesAnoAnterior = graficoGeral.exames_mensal?.filter(
    (item) => item.ano === new Date().getFullYear() - 1
  );
    ...
    datasets: [
      {
        ...
        data: (() => {
          // Filtrar os dados uma vez para o ano atual
          const pacientesFiltradosAnoAtual = filtrarDadosPorModalidade(
            pacientesAnoAtual,
            modalidadesSelecionadas
          );
      ...
      },
      {
        ...
        data: (() => {
          // Filtrar os dados uma vez para o ano anterior
          const pacientesFiltradosAnoAnterior = filtrarDadosPorModalidade(
            pacientesAnoAnterior,
            modalidadesSelecionadas
          );
          ...
          return meses.map((_, index) => {
            if (index === 0) {
              const totalPacientesMesAnterior = pacientesFiltradosAnoAnterior
                .filter((item) => getMonthIndex(item.mes) === 11)
                .reduce((acc, item) => acc + (item.total_pacientes || 0), 0);
              return totalPacientesMesAnterior === 0
                ? null
                : totalPacientesMesAnterior;
            } else {
              const totalPacientesMesAnterior = pacientesFiltradosAnoAnterior
                .filter((item) => getMonthIndex(item.mes) + 1 === index)
                .reduce((acc, item) => acc + (item.total_pacientes || 0), 0);
  ...
  const valorAnoAtual = graficoGeral.exames_mensal?.filter(
    (item) => item.ano === new Date().getFullYear()
  );
  const valorAnoAnterior = graficoGeral.exames_mensal?.filter(
    (item) => item.ano === new Date().getFullYear() - 1
  );
    ...
    datasets: [
      {
        ...
        data: (() => {
          // Filtrar os dados uma vez para o ano atual
          const valorFiltradoAnoAtual = filtrarDadosPorModalidade(
            valorAnoAtual,
            modalidadesSelecionadas
          );
          // Mapear os dados mensais usando o array filtrado
            ...
            const totalValorMes =
              index === 0
                ? valorFiltradoAnoAtual
                    .filter((item) => getMonthIndex(item.mes) === 11)
                    .reduce(
                      (acc, item) => acc + (parseFloat(item.total_valor) || 0),
                      0
                    )
                : valorFiltradoAnoAtual
                    .filter((item) => getMonthIndex(item.mes) + 1 === index)
      ...
      },
      {
        ...
        data: (() => {
          // Filtrar os dados uma vez para o ano anterior
          const valorFiltradoAnoAnterior = filtrarDadosPorModalidade(
            valorAnoAnterior,
            modalidadesSelecionadas
          );
          // Mapear os dados mensais usando o array filtrado
          return meses.map((_, index) => {
            const totalValorMesAnterior =
              index === 0
  ...
  return (
    <div className="comercial-geral-container">
      <div className="comercial-geral-filtro">
        <label className="comercial-geral-filtro-label">
          Filtro de Modalidades
        </label>
        <div className="comercial-geral-filtro-modalidades">
          {modalidadesDisponiveis.map((modalidade) => (
            <label className="comercial-geral-filtro-modalidades-cb" key={modalidade}>
              <input
                type="checkbox"
